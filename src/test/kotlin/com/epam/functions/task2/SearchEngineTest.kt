...
import com.epam.functions.task2.engine.SearchResult
...
internal class SearchEngineTest {
    ...
    @Test
    fun `input '1' (contains) should return only 2 tv channels (1+1 and Football 1)`() = testScope.runBlockingTest {
        val testQuery = "1"
        val result = invokeRequest(testQuery)
        val expectedTitles = listOf("1+1", "Football 1")
        val actualTitles = result.flatMap { it.assets.map { it.getPoster() } }
        assertTrue("Only ONE LIVE content item should be found") {
