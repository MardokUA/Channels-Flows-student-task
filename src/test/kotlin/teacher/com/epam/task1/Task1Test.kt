...
class Task1Test {
    ...
    fun testActorBodyLine() {
        runBlocking {
            ...
            //then
            Assert.assertThat(
                "actor in createBodyLine doesn't return correct log, please check your implementation",
    ...
    fun testActorEquipmentLine() {
        runBlocking {
            ...
            //then
            Assert.assertThat(
                "actor in createEquipmentLine doesn't return correct log, please check your implementation",
                outputStreamCaptor.toString().trim(), CoreMatchers.containsString(line)
            )
            equipmentLine.close()
        }
    ...
    fun testSelectOnCombineBodyWithLineTwo() {
        runBlocking {
            ...
            //then
            Assert.assertThat(
                "select in combineBody has to work with $combineBodyBodyLine2 but in your implementation it doesn't, and it is sadly ",
    ...
    fun testSelectOnCombineBodyWithLineOne() {
        runBlocking {
            ...
            //then
            Assert.assertThat(
                "select in combineBody has to work with $combineBodyBodyLine1 but in your implementation it doesn't, and it is sadly ",
    ...
    fun testSelectOnCombineEquipmentWithLineTwo() {
        runBlocking {
            ...
            //then
            Assert.assertThat(
                "select in combineEquipment has to work with $combineEquipmentEquipmentLine2 but in your implementation it doesn't, and it is sadly ",
    ...
    fun testSelectOnCombineEquipmentWithLineOne() {
        runBlocking {
            ...
            //then
            Assert.assertThat(
                "select in combineEquipment has to work with $combineEquipmentEquipmentLine1 but in your implementation it doesn't, and it is sadly ",
                outputStreamCaptor.toString().trim(),
    ...
    fun testIsThatOrderedIsProduced() {
        runBlocking {
            ...
            Assert.assertThat(
                "result output doesn't contain first ordered element",
                outputStreamCaptor.toString().trim(),
    ...
    fun testIsConstructorsTeamsWorkInParallel() {
        runBlocking {
            ...
            startWorkShopWork(orders)
            Assert.assertThat(
                "Constructors team 1 doesn't take part in the process ",
                outputStreamCaptor.toString().trim(),
                CoreMatchers.containsString(ProvidedByConstructorTeam1)
            )
            Assert.assertThat(
                "Constructors team 2 doesn't take part in the process ",
                outputStreamCaptor.toString().trim(),
